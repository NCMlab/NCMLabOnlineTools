...
var jsPsychSketchpadLineBisection = (function (jspsych) {
  'use strict';
  ...
  class SketchpadPlugin {
      ...
      trial(display_element, trial, on_load) {
          ...
          this.Lines = this.convert_locations(this.params.Lines, LocationsAsProportions, this.params.canvas_width, this.params.canvas_height)
          for (var i = 0; i < this.params.Lines.length; i++){            
            this.add_line(this.params.Lines[i].LeftX, this.params.Lines[i].LeftY,this.params.Lines[i].RightX,this.params.Lines[i].RightY)
          }
      ...
      convert_locations(Lines, LocationsAsProportions, WindowX, WindowY)
      {
        if (LocationsAsProportions)
        {
          for ( var i = 0; i < Lines.length; i++)
          {
            Lines[i].LeftX = Lines[i].LeftX*WindowX;
            Lines[i].LeftY = Lines[i].LeftY*WindowX;
            Lines[i].RightX = Lines[i].RightX*WindowX;
            Lines[i].RightY = Lines[i].RightY*WindowX;
          }
        }
        return Lines;
      }
