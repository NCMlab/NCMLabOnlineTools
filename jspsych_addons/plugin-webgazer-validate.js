var jsPsychWebgazerValidate = (function (jspsych) {
  ...
  class WebgazerValidatePlugin {
      ...
      trial(display_element, trial) {
          ...
          var html = `
        <div id='webgazer-validate-container' style='position: relative; width:80vw; height:80vh; overflow: hidden;'>
        </div>`;
        ...
        `;
          ...
          function calculateGazeCentroid(gazeData) {
              ...
              var median_distance = median(gazeData.map((x) => Math.sqrt(Math.pow(x.dx - x_diff_m, 2) + Math.pow(x.dy - y_diff_m, 2))));
              return {
                  x: x_diff_m,
                  y: y_diff_m,
                  r: median_distance,
  ...
  WebgazerValidatePlugin.info = info;
