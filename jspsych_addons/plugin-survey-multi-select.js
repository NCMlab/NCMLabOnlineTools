  ...
  const info = {
      ...
      parameters: {
          ...
          colCount: {
            ...
            pretty_name: "Column Count",
            default: 1,
          },
          /** If true, a free-text input is shown below each question's option list. */
          showOtherOption: {
            type: jspsych.ParameterType.BOOL,
            pretty_name: "Show Other Option",
            default: false,
          },
          /** Label shown next to the free-text "other" input, when showOtherOption is true. */
          otherOptionText: {
            type: jspsych.ParameterType.STRING,
  ...
  class SurveyMultiSelectPlugin {
      ...
      trial(display_element, trial) {
          ...
          var cssstr = ".jspsych-survey-multi-select-question { margin-top: 2em; margin-bottom: 2em; text-align: left; }" +
              ...
              ".jspsych-survey-multi-select-other { margin-top: 1em; }" +
              ".jspsych-survey-multi-select-other label {margin-right: 1em;}" +
              ".jspsych-survey-multi-select-other input[type='text'] {width: 30em; max-width: 100%; height: 2.5em; font-size: inherit; box-sizing: border-box;}";
          ...
          for (var i = 0; i < trial.questions.length; i++) {
              ...
              display_element.querySelector(question_selector).innerHTML +=
                  '<p id="survey-question" class="' +
                      ...
                      question.prompt +
                      "</p>";
              // create options container so checkboxes can be laid out in columns
              var options_container_id = _join(plugin_id_name, "options", question_id);
              display_element.querySelector(question_selector).innerHTML +=
                  '<div id="' + options_container_id + '" class="' + _join(plugin_id_name, "options") + '"></div>';
              var options_container = document.getElementById(options_container_id);
              if (trial.colCount > 1) {
                  options_container.style.display = "grid";
                  options_container.style.gridTemplateColumns = "repeat(" + trial.colCount + ", 1fr)";
              }
              ...
              // add free-text "other" input below the option list
              if (trial.showOtherOption) {
                  var other_input_id = _join(plugin_id_name, "other", question_id);
                  display_element.querySelector(question_selector).innerHTML +=
                      '<div class="' + _join(plugin_id_name, "other") + '">' +
                          '<label for="' + other_input_id + '">' + trial.otherOptionText + "</label>" +
                          '<input type="text" id="' + other_input_id + '" name="' + other_input_id + '">' +
                      "</div>";
              }
          }
          // add submit button
